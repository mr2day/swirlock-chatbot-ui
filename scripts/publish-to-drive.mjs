 ...
 * in the APK (version, build date, last 5 UI commit messages).
 *
 ...
 * deploy.
 *
 * Destination path is hard-coded to G:\My Drive\Claude because
 * that's where the user created the folder. Change the constant
 * below if Drive ever moves.
 */
...
function main() {
  ...
  // Orchestrator latest commits — when wiring changed there too.
